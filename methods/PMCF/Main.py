...
class Coach:
	...
	def run(self):
		self.prepareModel()
		try:
			self.pprMat = pickle.load(open('ppr_' + args.data + '.pkl', 'rb'))
			self.entropy = pickle.load(open('entropy_' + args.data + '.pkl', 'rb')).view([-1, 1])
	...
	def trainEpoch(self):
		...
		for i, tem in enumerate(trnLoader):
			if i % args.fixSteps == 0:
				sampScores, seeds = self.sampler(self.handler.allOneAdj,self.handler.adj_2, self.model.getEgoEmbeds(), self.pprMat,self.entropy)
